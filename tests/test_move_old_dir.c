...
Test(move_old_dir, with_oldpwd_set_returns_success)
{
    env_t env = {0};
    ...
    int ret = move_old_dir(&env);

    cr_assert_eq(ret, SUCCESS);
    free_arr(env.ev_vars);
}

Test(move_old_dir, no_oldpwd_returns_error)
{
    env_t env = {0};
...
Test(move_old_dir, empty_env_returns_error)
{
    env_t env = {0};
